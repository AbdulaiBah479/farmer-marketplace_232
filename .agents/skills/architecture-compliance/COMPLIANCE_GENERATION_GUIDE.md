...
**Purpose**: Define guidelines for software development including technology lifecycle (EOL) tracking, development best practices, and architecture debt management through standardized compliance requirements.
...
- **EOL Validation**: Automatic per-technology lifecycle lookup via WebSearch against `endoflife.date` (one DEV-EOL-{N} item per technology+version pair found in Section 8)
- **Requirement**: All technology choices must comply with organizational standards and use supported (non-EOL) versions
...
- **Primary**:
  - Section 8 (Technology Stack) - 60% of content
    ...
    - EOL Validation per technology+version pair (DEV-EOL-{N})
...
**Example Guidelines**:
- Technology stack should document explicit versions for every component (the validator skips entries without versions)
- EOL or near-EOL (within 6 months) versions trigger FAIL status (LADES1)
...
- EOL Validation is MANDATORY — contract cannot be approved with EOL or near-EOL technologies that lack an LADES2 exception
- EOL findings automatically populate LADES2.1 for exception review

**Automatic EOL Validation**: This contract includes **mandatory** automatic lifecycle validation during document generation. The development-validator extracts every technology+version pair from `docs/06-technology-stack.md` and runs WebSearch lookups against `endoflife.date`. Results are embedded as one DEV-EOL-{N} item per pair with PASS (>6 months remaining), FAIL (EOL or within the 6-month safety period), or UNKNOWN (lookup inconclusive) status. FAIL status requires LADES2 exception documentation before contract approval.

> **Scope note (v3.22.0)**: The previous 26-item Stack Validation Checklist (Java/.NET/Frontend/Other approved-tech lists, naming conventions, library catalog) was removed because it relied on hardcoded organizational lists that drifted from reality and could not be queried live. Stack-shape validation will return when a live mechanism (MCP server, organizational catalog API, or registry) is integrated.

**Template Priority**: Medium (Template #5)
**Validation Items**: Dynamic — one DEV-EOL-{N} item per technology+version pair + 2 LADES requirements
...
DON'T:
- Read all 11 docs/ files for every contract
...
## Edge Case Handling for Automatic EOL Validation

### Overview

Automatic EOL validation (executed by `agents/validators/development-validator.md` via WebSearch lookups against `endoflife.date`) handles a small set of structural edge cases. The previous "Stack Validation" edge cases (backend-only, full-stack, polyglot, unapproved technology, etc.) no longer apply because the validator no longer compares the documented stack against an organizational approved-tech list — it only evaluates lifecycle status per technology+version pair.

### Edge cases the EOL validator handles

| Edge Case | Detection | Validator Behavior | Overall Status |
|-----------|-----------|--------------------|----------------|
| `docs/06-technology-stack.md` missing or <50 chars | Read fails or content trivially short | Emit `total_items: 0` with note that no stack is documented | FAIL (the orchestrator treats missing primary source as a structural failure) |
| Tech mentioned without explicit version | Grep finds a name but no adjacent version | Skip (no DEV-EOL row); record in recommendations as "version not documented" | Does not affect status |
| Tech with version, EOL >6 months away | WebSearch + endoflife.date | DEV-EOL-N → PASS | Contributes PASS |
| Tech with version, EOL within 6 months or already past | WebSearch + endoflife.date | DEV-EOL-N → FAIL with deviation entry | Contributes FAIL |
| Tech with version, no conclusive EOL data | WebSearch returns nothing definitive | DEV-EOL-N → UNKNOWN with recommendation to verify manually | Does not block (UNKNOWN does not fail the contract) |
| WebSearch tool unavailable | Tool error / not configured | All items UNKNOWN, status PASS, recommend manual verification | Does not block (degrade gracefully) |

### What the validator no longer does (removed in v3.22.0)

The 26-item Stack Validation Checklist (Java/.NET/Frontend backend approved frameworks, library catalog comparison, naming conventions, build tooling catalog, polyglot detection, "unapproved technology" detection) was removed because it depended on hardcoded organizational lists baked into the validator prompt. Those lists drifted from reality and could not be queried live. Stack-shape validation will return when a live mechanism (MCP server, organizational catalog API, or registry) is integrated; until then, EOL is the sole external check.

If a project needs stack-shape compliance enforcement before that mechanism arrives, document deviations manually in LADES2 (Exception Handling).
