...
12. **S13: Risks and Technical Debt** (`docs/10-risks-and-technical-debt.md`) - Known risks, accepted technical debt; summary view linking to `/tdr/`
...
| S13 (Risks and Technical Debt) | Known risks, accepted technical debt | `docs/10-risks-and-technical-debt.md` |
| References | External references | `docs/11-references.md` |
...
#### Document Purpose
Define development architecture standards including design patterns, technology lifecycle (EOL) tracking, component design, and development best practices.
...
- Section 8: Technology stack (languages, frameworks, versions) → drives **EOL Validation**
- Section 12: Technology decision ADRs, trade-offs
- Section 11: CI/CD pipelines, deployment processes

**Special Feature — EOL Validation (v3.22.0)**: The development-validator extracts every technology+version pair from `docs/06-technology-stack.md` and performs a WebSearch lookup against `endoflife.date`. It emits one `DEV-EOL-{N}` row per pair with PASS (>6 months remaining), FAIL (EOL or within the 6-month safety period), or UNKNOWN (lookup inconclusive) status. The result is a blocking gate for contract approval (FAIL items must be resolved by upgrade or via an LADES2 exception).

> **Scope note**: The previous 26-item Stack Validation Checklist (Java/.NET/Frontend approved-tech lists, library catalog, naming conventions) was removed in v3.22.0 because it relied on hardcoded organizational lists that drifted from reality. Stack-shape validation will return when a live mechanism (MCP server, organizational catalog API, or registry) is integrated.

#### Missing Data Handling

```
Scenario 1: docs/06-technology-stack.md missing or empty
Action: Validator emits total_items: 0; orchestrator treats missing primary source as a structural failure.

Scenario 2: Partial Section 8 (tech named but no version)
Action: Skip the entry (no DEV-EOL row); record in recommendations as "version not documented".

Scenario 3: WebSearch returns no conclusive EOL data
Action: Mark DEV-EOL-{N} as UNKNOWN. UNKNOWN does not fail the contract — emit a recommendation for manual verification.

Scenario 4: WebSearch tool unavailable
Action: All items UNKNOWN, status PASS, recommend manual verification (degrade gracefully).
```
...
- [ ] `docs/11-references.md` exists (if references section is needed)
