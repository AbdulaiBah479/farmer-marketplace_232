...
- **Reason**: EOL Validation (via WebSearch against `endoflife.date`) is a blocking requirement
- **Unique Logic**: Contract cannot be approved while any documented technology+version is EOL or within the 6-month safety period without an LADES2 exception
