...
<!--
COMPONENT NAMING FIDELITY (authoring rule for the handoff-generator — do not render)

When this template is filled in, every reference to a documented component
in any Section (Metadata, Dev tracks A1–A3, QA B1, Ops C1–C3, Appendix)
MUST use the canonical full name exactly as it appears in
`docs/components/README.md` (or the `ARCHITECTURE.md` Component Index if no
separate README exists). Do NOT abbreviate, truncate, or alias the name
even when the source component file or `docs/**/*.md` uses a shortened form
inline. Example: write `omn-bs-top-ups-and-bundles` every time — never
`omn-bs` or `top-ups-and-bundles` standalone. The [COMPONENT_NAME]
placeholder MUST be filled with the canonical full name; downstream
references (filename slugs, asset paths, prose mentions, dependency rows)
must propagate that exact name without shortening.
-->
