...
**Mode-aware extraction.** Detect mode by grepping `docs/04-data-flow-patterns.md` for the literal H2 strings `## Phase Catalog`, `## Flow Inventory`, `## Use Case Resolutions` — any two present = **Phase Catalog mode**, otherwise **Single-Flow mode**.

**Phase Catalog mode**:

For each phase H4 under `## Phase Catalog` whose `sequenceDiagram` lists this component as a `participant`, include verbatim:

- The phase H4 header.
- The `**Purpose**`, `**Latency budget (p95)**`, and `**Side effects**` lines.
- The phase's `sequenceDiagram` block.

Then, for each UC under `## Use Case Resolutions` whose phase composition includes any of the included phases, include the UC's narrative delta paragraph (the H4 + the prose that follows, up to the next H3/H4).

Then, append every row of the `### Failure classification` table whose `Phase` column references any of the included phases.

**Do NOT include `## End-to-End Wire Sequences`** in this payload — they are a navigation aid for the full doc, and inlining a 200-line wire sequence into every component handoff re-introduces the cross-flow duplication the Phase Catalog pattern eliminates. The per-phase mini-flows above already give the dev team everything they need to implement this component's role.

**Single-Flow mode**:

Include all flows from `docs/04-data-flow-patterns.md` where this component appears as a participant — the full flow description (prose + any sequence diagrams in the flow's block).

**If no flows reference this component in either mode**, write:
