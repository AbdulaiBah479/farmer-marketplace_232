...
7. **Component name resolution (Fidelity Rule)**: Several extraction rules below read component references from sources that may use shortened forms — notably `docs/05-integration-points.md` (Upstream Consumers, Downstream Dependencies, Async Integrations, External System Integrations rows in A1/A3) and `**Dependencies:**` / `**Calls:**` fields inside component files (Out of Scope, Downstream Dependencies in A1; Failure Recovery in C2). Whenever an extracted value names another documented component, you MUST resolve that name to its **canonical full name** from `docs/components/README.md` Column 2 (or the `ARCHITECTURE.md` Component Index if no separate README exists) **before writing it into the handoff**. The source may say `omn-spa` or `omn-bs`; if the canonical name is `omn-spa-product-summary` or `omn-bs-top-ups-and-bundles`, write the full form. This is not optional and applies to every row, table, and prose mention — never propagate the shortened form even if the source uses it inline. This rule is the per-field enforcement of the agent-level Component Naming Fidelity block at the top of this system prompt.
...
| Async Integrations | `docs/04-data-flow-patterns.md` | `docs/05-integration-points.md` | Event/message flows involving this component — topic, role (producer/consumer), format, schema. **Mode-aware**: in Phase Catalog mode, scan every phase H4's `sequenceDiagram` for `-)` async edges where this component is source or target, plus the matching row in the failure classification table. In Single-Flow mode, scan every H3 flow for the same. Do **not** include the End-to-End Wire Sequences — they duplicate the per-phase content this row already captures. |
