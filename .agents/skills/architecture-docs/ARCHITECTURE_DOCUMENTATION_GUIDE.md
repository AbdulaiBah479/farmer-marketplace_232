...
> Internal section numbers (S1-S13) identify content concepts. File prefix numbers (01-11) are filesystem ordering only. They do NOT align.
...
### 1.5 Enterprise Domain Alignment

> **Source**: lifted from `ENTERPRISE_ALIGNMENT.md` at the project root, generated by `sa-skills:architecture-enterprise-alignment` and accepted by the Workflow 1 Step 0.3 gate.

**Enterprise Model**: [bundled default (sa-skills v[plugin-version]) | project-local override (ENTERPRISE_MODEL.md)]
**Alignment Report**: [`ENTERPRISE_ALIGNMENT.md`](../ENTERPRISE_ALIGNMENT.md)
**Alignment Generated**: YYYY-MM-DD

**Impacted Domains** (lifted verbatim from `ENTERPRISE_ALIGNMENT.md` § Summary — show only rows where Status is 🟢 Impacted or 🟡 Partial; omit ⚪ rows for brevity):

| # | Domain | Status | Capabilities Impacted | Rules Triggered | Owners to Engage |
|---|--------|--------|-----------------------|-----------------|------------------|
| [N] | [domain] | 🟢 / 🟡 | N/total | N | [roles] |

**Impacted Capabilities (per domain)** — lifted from `ENTERPRISE_ALIGNMENT.md` per-domain `**Impacted Capabilities**` blocks (🟢 / 🟡 only; ⚪ domains omitted; the `(not impacted: …)` parenthetical is dropped here for brevity — see the source report for the full enumeration):

- **[Domain]** (N/total): `[Capability A]` 🟢, `[Capability B]` 🟡, `[Capability C]` 🟢
- **[Domain]** (N/total): `[Capability D]` 🟢
- …

**Cross-Cutting Findings** (one line each, lifted from `ENTERPRISE_ALIGNMENT.md` § Cross-Cutting Findings):
- **Risks** — ✅ Engagement planned / 🔴 Gap: [one-line summary]
- **IT** — ✅ Aligned / 🔴 Gap: [one-line summary]
- **Data** — ✅ Aligned / 🔴 Gap: [one-line summary]

**Owner Engagement Plan** — see `ENTERPRISE_ALIGNMENT.md` § Owner Engagement Plan for the full table. The architect MUST capture sign-off references (tickets, ARB minutes, emails) before this architecture moves from Draft to Released.
```

**Rules for Section 1.5**:
- This sub-section is populated **only** during Step 5 of Workflow 1 when Step 0.3 PASSED. If Step 0.3 was cascading-skipped (because PO Spec was skipped), omit Section 1.5 entirely and replace it with the comment `<!-- ENTERPRISE_ALIGNMENT_GATE: SKIPPED -->` so downstream skills can detect the absence.
- The Section 1.5 table is **NOT** the source of truth for the alignment — `ENTERPRISE_ALIGNMENT.md` is. Section 1.5 is a summary lift. To refresh, re-run `/sa-skills:architecture-enterprise-alignment` and then re-run the affected portion of `architecture-docs` (typically Workflow 2 — Edit Existing Section, targeting Section 1.5).
- The component-naming-fidelity rule applies: use the verbatim domain names from `ENTERPRISE_ALIGNMENT.md` (e.g., `Finance & Risks`, not `Risks`; `Business Enabler`, not `IT/Data`) in Section 1.5 cells.
- **Capability names** must be lifted verbatim from `ENTERPRISE_ALIGNMENT.md` per-domain `**Impacted Capabilities**` blocks — no abbreviation, no aliasing (e.g., write `Customer Agreement & Contract Management`, not `Customer Contracts`; write `Money Movement Management`, not `Money Movement`).
- **Version-aware fallback**: if `ENTERPRISE_ALIGNMENT.md` carries `<!-- ENTERPRISE_ALIGNMENT_VERSION: 1.0 -->` or `2.0 -->` (pre-capability-detail format), Section 1.5 falls back to the legacy 5-column table (`# | Domain | Status | Rules Triggered | Owners to Engage`) and **omits** the `Impacted Capabilities (per domain)` block. Users who want the capability-level lift re-run `/sa-skills:architecture-enterprise-alignment` to regenerate at v2.1.
...
**ID format**: `ADR-NNN` where `NNN` is a 3+ digit, zero-padded number within the row's Scope range — institutional (`001–100`, Architecture Team) or user/project (`101+`, this project). **Never** include phase prefixes (`ADR-P1-001`), version suffixes (`ADR-101-v2`), variant letters (`ADR-101a`), or any non-digit characters in the body — the 100+ range encodes project scope on its own. The full format rules and anti-patterns are in `ADR_GUIDE.md § ADR ID Format Rules`; the regex gate that rejects malformed IDs lives in `architecture-definition-record/SKILL.md § ADR ID Format Gate`.
...
|----|-------|-------|--------|------|--------|
| [ADR-101](adr/ADR-101.md) | [Title] | User | Accepted | YYYY-MM-DD | High/Medium/Low |
```

> **Scope column**: `Institutional` for org-wide standards owned by the Architecture Team (IDs 001–100); `User` for decisions local to this project (IDs 101+). The column is the authoritative scope source for downstream skills — `architecture-definition-record` Workflow 1 Step 1.3 reads it directly when present and skips the inference dance.
...
## Section 13: Risks and Technical Debt

**Purpose**: Surface known architectural risks and accepted technical debt so they remain visible and reviewable. Risks describe potential future problems; technical debt describes accepted present compromises.

Full Technical Debt Records (TDRs) live under `tdr/` at the repository root and are owned by the `technical-debt-records` skill. This section is a *summary view* that links into those records — never duplicate full TDR content here.

**Source File** (multi-file layout): `docs/10-risks-and-technical-debt.md`

**Auto-create when missing**: When this section's file is absent during Workflow 1 (new doc creation) or Workflow 9 (multi-file restructure), create `docs/10-risks-and-technical-debt.md` from the template below. The summary tables start empty in new architectures and are populated as TDRs are recorded via the `technical-debt-records` skill.
...
This section lists known risks and technical debt affecting the architecture. Full Technical Debt Records (TDRs) are maintained under [`/tdr/`](../tdr/) at the repository root; this section provides a summary view.

### 13.1 Risks

<!-- Risks describe potential future problems; technical debt (13.2) describes accepted present compromises. -->
...
Open technical debt items, summarized. See [`/tdr/`](../tdr/) for the full records.

| ID      | Title                                       | Status | Owner    |
|---------|---------------------------------------------|--------|----------|
| TDR-NNN | <short title>                               | Open   | <team>   |

### Conventions

- TDRs are created using the `technical-debt-records` skill workflow.
- IDs are global and zero-padded (`TDR-NNN`) and never reused.
- Status values: `Open`, `Planned`, `In Progress`, `Resolved`, `Won't Fix`.
- When a TDR is `Resolved` or `Won't Fix`, it remains in `/tdr/` for traceability but is removed from this summary table.
```

> **arc42 alignment**: external arc42 calls this Section 11. This skill's internal numbering puts it at **Section 13** to avoid colliding with the existing Section 11 (Operational Considerations). Projects migrating from arc42 should map their Section 11 → this Section 13. The drop-in arc42 snippet shipped by the TDR skill (`skills/technical-debt-records/references/arc42-section-11-snippet.md`) is for arc42 layouts that store docs under `docs/architecture/`; for this skill's `docs/` layout, use the template above.

For the full TDR format and creation workflow, see [`technical-debt-records` skill](../technical-debt-records/SKILL.md).
