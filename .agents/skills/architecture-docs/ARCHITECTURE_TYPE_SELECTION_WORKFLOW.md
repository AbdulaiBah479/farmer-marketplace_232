...
#### Step 0.3: Enterprise Alignment Gate (MANDATORY — no skip)

After the PO Spec gate resolves successfully, verify that an **Enterprise Alignment** artifact exists.

This gate ensures every new ARCHITECTURE.md is preceded by a formal mapping of the initiative onto the organization's BIAN v14 capability map (Enterprise Management, Resource Management, Finance & Risks, Operations, Products, Customers, Channels, Business Enabler — 47 capabilities across 8 domains). Owner counterparts must be identified before design begins.

**Scope**: This gate runs **only for Workflow 1 (new ARCHITECTURE.md creation)**. It does NOT run for Workflow 2 (edits), Workflow 8 (diagrams), Workflow 9 (migration), or Workflow 10 (release).

**Skip path — PO Spec was skipped**: If `<!-- PO_SPEC_GATE: SKIPPED -->` was just recorded (Step 0 Option 4), automatically skip this gate as well — enterprise alignment requires PO Spec content to map. Record:

```
<!-- ENTERPRISE_ALIGNMENT_GATE: SKIPPED (cascading from PO_SPEC_GATE: SKIPPED, YYYY-MM-DD) -->
```

…and proceed to Step 0.4. Otherwise:

**Detection procedure** (ordered — stop at first match):
1. Check for `ENTERPRISE_ALIGNMENT.md` at project root
2. Glob search: `**/ENTERPRISE_ALIGNMENT.md`

---

**If ENTERPRISE_ALIGNMENT.md FOUND → Gate passes:**

- Display: `✅ Enterprise Alignment found: [filename]`
- Parse the file's frontmatter HTML comments:
  - `<!-- ENTERPRISE_ALIGNMENT_VERSION: X.Y -->`
  - `<!-- ENTERPRISE_MODEL_SOURCE: ... -->`
  - `<!-- PO_SPEC_SOURCE: [filename] -->`
  - `<!-- GENERATED: YYYY-MM-DD -->`
- **Staleness check**: If `PO_SPEC_SOURCE` filename differs from the PO Spec located in Step 0, OR if `GENERATED` date is older than the PO Spec's mtime, display a warning:
  ```
  ⚠️  Enterprise Alignment may be stale.
     Generated: YYYY-MM-DD against [PO_SPEC_SOURCE]
     Current PO Spec: [Step 0 filename] (modified YYYY-MM-DD)
     Consider re-running /sa-skills:architecture-enterprise-alignment before proceeding.

     Continue anyway? (y/n)
  ```
  Wait for user response. On `n`, abort and direct to the alignment skill. On `y`, proceed.
- Load the alignment file into context for use during Step 5 — specifically the **Summary table** (including the `Capabilities Impacted` column when present in v2.1+ files), the per-domain **Impacted Capabilities** bullet lists (v2.1+), the **Cross-Cutting Findings**, and the **Owner Engagement Plan**. These are lifted into a new sub-section `1.5 Enterprise Domain Alignment` in `docs/01-system-overview.md` (Section 1 Executive Summary). When the file carries `<!-- ENTERPRISE_ALIGNMENT_VERSION: 1.0 -->` or `2.0 -->`, fall back to the legacy domain-only Section 1.5 lift per `ARCHITECTURE_DOCUMENTATION_GUIDE.md` § 1.5 version-aware fallback rule.
- Record traceability metadata (embed in `docs/01-system-overview.md` during Step 5):
  ```
  <!-- ENTERPRISE_ALIGNMENT_GATE: PASSED (ref: ENTERPRISE_ALIGNMENT.md, YYYY-MM-DD) -->
  <!-- ENTERPRISE_ALIGNMENT_SOURCE: ENTERPRISE_ALIGNMENT.md -->
  ```
- Proceed to Step 0.4

---

**If ENTERPRISE_ALIGNMENT.md NOT FOUND → Gate BLOCKS:**

Display the following message and STOP — there is no `SKIP ENTERPRISE ALIGNMENT` override at this gate (unlike Step 0). The only way past this gate is to run the alignment skill or, if absolutely necessary, skip the PO Spec gate entirely (which cascades-skip this gate too).

```
🚫 No Enterprise Alignment Found

The four-phase workflow requires enterprise-domain mapping before architecture design:

  Phase 1   → Product Owner Specification         ✅
  Phase 1.5 → Enterprise Alignment                ← MISSING (you are here)
  Phase 2   → ARCHITECTURE.md                     ← Blocked
  Phase 3   → Compliance Contracts
  Phase 4   → Component Development Handoff

Enterprise Alignment maps your initiative onto the 8-domain BIAN v14 capability
map (Enterprise Management, Resource Management, Finance & Risks, Operations,
Products, Customers, Channels, Business Enabler — 47 capabilities total) and
identifies which corporate owner counterparts must be engaged before
architecture design begins.

Run this command and re-invoke /sa-skills:architecture-docs afterwards:

  /sa-skills:architecture-enterprise-alignment

It will produce ENTERPRISE_ALIGNMENT.md at the project root using the PO Spec
located above and the bundled Enterprise Model (overridable via a project-local
ENTERPRISE_MODEL.md at the project root).
```

After displaying, **STOP**. Do NOT proceed to Step 0.4. Do NOT auto-invoke the alignment skill — the user explicitly runs it so they remain in control of the cadence.

---

#### Step 0.4: HLD Intake Gate (MANDATORY — no skip)

After the Enterprise Alignment Gate resolves successfully, prompt the user once for the project's **High-Level Design (HLD)** file. This gate is mandatory — there is no `SKIP HLD` override. The HLD anchors S4 (Architecture Layers), S5 (Component Inventory), and S6 (Data Flow Patterns) so they reflect upstream design intent rather than reverse-derivation from the PO Spec alone.

**Scope**: This gate runs **only for Workflow 1 (new ARCHITECTURE.md creation)**. It does NOT run for Workflow 2 (edits), Workflow 8 (diagrams), Workflow 9 (migration), or Workflow 10 (release).

**Cascades-skip rule**: If `<!-- PO_SPEC_GATE: SKIPPED -->` OR `<!-- ENTERPRISE_ALIGNMENT_GATE: SKIPPED -->` was recorded earlier in this workflow, automatically skip this gate too — there is no design intent to anchor against. Record:

```
<!-- HLD_GATE: SKIPPED (cascading from upstream gate skip, YYYY-MM-DD) -->
```

…and proceed to Step 0.5. Otherwise, display:

```
📐 **HLD Intake Gate (Required)**

This architecture must be anchored to a High-Level Design document.
Provide the path to your HLD file. Accepted formats:

  • PDF  (.pdf)
  • SVG  (.svg)            — exported from draw.io, Lucidchart, Excalidraw
  • PNG  (.png)            — high-resolution recommended (≥1920px wide
                             or ≥150 DPI) for diagram label readability

Examples:  design/hld.pdf  •  diagrams/system-overview.svg  •  hld.png

Type the file path to continue. There is no skip option.
```

**Processing the HLD file**:

1. **Validate the extension** is one of `.pdf`, `.svg`, `.png` (case-insensitive). Reject any other extension and re-prompt:
   ```
   ⚠️ [filename] is not a supported HLD format.
   Accepted: .pdf, .svg, .png. Please provide an HLD file in one of these formats.
   ```
   Continue re-prompting until a valid file is supplied. Do NOT proceed to Step 0.5 without a valid HLD.

2. **Read the file**:
   - `.pdf` → Read tool with the `pages` parameter (chunks of 20 pages for PDFs >10 pages)
   - `.png` → Read tool (multimodal — Claude sees the image visually)
   - `.svg` → Read tool (XML text — Claude parses element labels, group hierarchy, and arrow markers from the markup)

3. **Extract design concepts** for downstream sections:
   - Component / container names → seed S5 (Component Inventory)
   - Layer / tier groupings → seed S4 (Architecture Layers)
   - Connection arrows / integration lines → seed S6 (Data Flow Patterns)
   - Annotated technologies → cross-check against S8 (Technology Stack)

4. **Add source attribution** everywhere HLD-derived content appears in `docs/**/*.md` during Step 5:
   ```
   <!-- HLD_SOURCE: [filename] -->
   ```
...
5. **Display confirmation**:
   ```
   ✅ HLD loaded: [filename] ([format])
      Will inform: S4 (Layers), S5 (Components), S6 (Data Flow)
   ```

6. **Record traceability metadata** (embed in `docs/01-system-overview.md` during Step 5):
   ```
   <!-- HLD_GATE: PASSED (ref: [filename], YYYY-MM-DD) -->
   <!-- HLD_SOURCE: [filename] -->
   <!-- HLD_FORMAT: pdf | svg | png -->
   ```

**Conflict resolution**:

- HLD vs PO Spec → PO Spec wins for *business* context; HLD wins for *structural* layout (component decomposition, integration topology).
- HLD vs supplementary files (Step 0.5) → HLD wins (it is the design anchor).
- HLD internally inconsistent (e.g. components on the diagram with no labels, or arrows with no source/target) → flag the gap and ask the user to clarify before continuing.

**Note on SVG visual fidelity**: SVG is parsed as XML text — Claude reads labels, group hierarchy, and arrow directions but does not "see" rendered visual positioning. For HLDs where positioning carries meaning (e.g. swimlane layouts, north-south vs east-west arrangement), prefer PDF or high-res PNG.
...
You can provide:
  • File paths (e.g., `specs/openapi.yaml`, `adr/ADR-001.md`)
...
#### Step 0.6: Pre-Identify Architecture Decision Records (ADR Context Block)
...
- **Do not assign ADR IDs at Step 0.6.** The `#` column is a row counter only. ADR IDs are computed at Step 6 (delegated to `architecture-definition-record`) per the ADR Scope Partition rule — **project ADRs start at 101**. **Never** invent IDs with phase / version / variant prefixes or suffixes (e.g., `ADR-P1-001`, `ADR-P2-002`, `ADR-Phase1-001`, `ADR-001-P1`, `ADR-101-v2`); they will be rejected by the **ADR ID Format Gate** in `architecture-definition-record/SKILL.md`. The 100+ range encodes project scope on its own — phase tracking in the ID is incorrect.
...
```
ADR CONTEXT BLOCK (confirmed at Step 0.6)
──────────────────────────────────────────
[Candidate 1] [Title]    | Driver: [source] | Status: PENDING
[Candidate 2] [Title]    | Driver: [source] | Status: PENDING
[Candidate 3] [Title]    | Driver: [source] | Status: PENDING
...
```

> Candidates carry **no ADR ID** until materialization. Workflow 1 Step 6 delegates to `architecture-definition-record`, which computes the next sequential number in the project scope (101+) and validates each ID against the ADR ID Format Gate before writing.

As decisions are made during Steps 1–5, update each candidate's status from `PENDING` to `DECIDED: [decision summary]` with the rationale captured in context. IDs (e.g., `ADR-101`, `ADR-102`) are assigned only at Step 6 — never during Steps 0.5 through 5.
...
**ADR Context Block consistency**: Reference the ADR Context Block (from Step 0.6) when writing each section. As decisions are made during section creation, update the corresponding ADR candidate from `PENDING` to `DECIDED` with the decision summary and rationale. This ensures:
...
3. Write each section to its corresponding `docs/NN-name.md` file (see RESTRUCTURING_GUIDE.md for file mapping):
   ...
   - `docs/11-references.md` — References
...
- **ADR Context Block**: Pass the full ADR Context Block from Step 0.6 — each ADR candidate now has its driver (PO Spec source), decision summary, rationale, and the section(s) where it was resolved. This enriched context ensures ADR files are complete and consistent with the architecture documentation.
