...
| SECTIONS-05 | Data Flow Patterns is well-formed (mode-aware) | Major | Detect mode by grepping `docs/04-data-flow-patterns.md` for the literal H2s `## Phase Catalog`, `## Flow Inventory`, `## Use Case Resolutions` — any two present = **Phase Catalog mode**, otherwise **Single-Flow mode**. **Phase Catalog mode**: the phase × UC matrix in `## Use Case Resolutions` is present AND its phase row count equals the phase row count in `## Phase Catalog` (silent drift between Phase Catalog and matrix = Major); `### Latency budget breakdown` table present; `### Failure classification` table covers ≥1 row per phase H4. **Single-Flow mode**: section contains at least one `mermaid` code block showing data flow; text-only descriptions = Major. |
