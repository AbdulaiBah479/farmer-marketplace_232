...
## Authoring Rules — Component Naming Fidelity

When the playground is generated and findings reference a documented
component by name (in finding text, severity tables, scorecard tooltips,
prompt output, exported reports), use the **canonical full name** exactly as
it appears in `docs/components/README.md` (or the `ARCHITECTURE.md`
Component Index if no separate README exists). Do not abbreviate, truncate,
or alias the name even when source `docs/**/*.md` files use a shortened form
inline. Example: write `omn-bs-top-ups-and-bundles` every time — never
`omn-bs` or `top-ups-and-bundles` standalone. This applies to every surface
the playground emits — finding bodies, prompt-output snippets, copy-to-
clipboard payloads, exported markdown — not just the visible UI.
