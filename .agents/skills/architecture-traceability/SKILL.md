...
1. Compute each row's exact percentage `pᵢ = 100 × countᵢ / total`.
2. Floor each: `fᵢ = floor(pᵢ)`. Track each remainder `rᵢ = pᵢ − fᵢ`.
3. Compute deficit `D = 100 − Σ fᵢ` (an integer in `0..N`, where `N` is the number of rows).
4. Sort rows by `rᵢ` descending; add `+1` to the top `D` rows.
5. Tie-break (`rᵢ` equal): larger `countᵢ` first, then row order in the table (Covered → Partial → Not Covered).

For **single-value cells** (per-phase `Coverage` column, per-UC `N/N (X%)` cell, any standalone percentage), use plain `round()` — there is no sum constraint.
...
- [ ] Global Summary: `Covered% + Partial% + NotCovered% == 100`.
- [ ] Each per-UC integer-% breakdown (if present): row sums to 100%.
- [ ] Each per-phase integer-% breakdown (if present): row sums to 100%.
- [ ] Single-value cells (overall per-UC `N/N (X%)`, per-phase `Coverage`) are exempt — `round()` is fine, no sum constraint.
- [ ] Counts reconcile in every breakdown: `Covered + Partial + NotCovered == Total` (catches off-by-one classification bugs the %-sum check alone cannot detect — largest-remainder will produce 100% even if the underlying counts are wrong).

If any check fails, recompute the offending row with the largest-remainder method before writing the report. Do not paper over a count mismatch by forcing percentages to 100 — that masks a real classification bug. Never ship a table whose integer-% rows sum to 99% or 101%.
