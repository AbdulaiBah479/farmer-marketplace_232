...
- `--assigned-to` accepts a UPN/email/display name. `ship-pr.ts` defaults it to `$SHIP_ADO_ASSIGNEE` (fallback `juliano.barbosa@hypera.com.br`); override per-run with `--assignee`.
...
- **`--assigned-to` must resolve to an org member** — an email/UPN that isn't a project member errors (or silently leaves the item unassigned on some orgs). The default `juliano.barbosa@hypera.com.br` is the `hyperadevops` identity; pass `--assignee` for any other org.
